...
#' dp_b48b01d6()
#' }
dp_b48b01d6 <- function(bbox = NULL, bbox_crs = NULL, timespan = NULL, ...) {
  ...
  if (!exist$clean) {
    ...
    # IMPORT DATA
    # =~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~= #
    utils::unzip(here::here(path, "raw", "98100233-eng.zip"), exdir = here::here(path, "raw"))
    dat <- vroom::vroom(here::here(path,"raw","98100233.csv"))
    # _________________________________________________________________________________________ #

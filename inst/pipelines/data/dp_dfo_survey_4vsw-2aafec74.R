#' @eval get_name("2aafec74")
#'
#' @eval get_description("2aafec74")
#'
#' @eval dp_params()
...
dp_2aafec74 <- function(bbox = NULL, bbox_crs = NULL, timespan = NULL, ...) {
  ...
  if (!exist$clean) {
    ...
    # IMPORT DATA
    # =~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~= #
    gscat <- utils::read.csv(here::here(path, "raw", "4VSW_2020_GSCAT.csv"))
    gsdet <- utils::read.csv(here::here(path, "raw", "4VSW_2020_GSDET.csv"))
    gsinf <- utils::read.csv(here::here(path, "raw", "4VSW_2020_GSINF.csv"))
    gsspecies <- utils::read.csv(here::here(path, "raw", "4VSW_2020_GSSPECIES.csv"))
    gsmissions <- utils::read.csv(here::here(path, "raw", "4VSW_2020GSMISSIONS.csv"))
    # _________________________________________________________________________________________ #

...
di_e2b7e6c4 <- function(output = "data", grid = NULL, fishing_intensity_metric = 1, ...) {
  ...
  logbooks <- dat[["fisheries_logbooks-f2109e69.geojson"]]
  ...
  if (is.null(grid)) {
    grd_pol <- sf::st_read("data/data-grid/grid_poly.geojson", quiet = TRUE) |>
               sf::st_transform(grd_pol, crs = sf::st_crs(logbooks))
  }
  ...
  # -----
  # LBS to KG
  uid <- logbooks$un_mes == "P"
  logbooks$pd_deb[uid] <- logbooks$pd_deb[uid] *  0.453592
  ...
  species <- table(logbooks$cod_esp) |>
             ...
             dplyr::select(ID = ESP_STAT, Scientific = DL_ESP, Espece = DF_ESP,
                           Species = DA_ESP, Freq)
  ...
  # -----
  logbooks <- dplyr::group_by(logbooks, date_cap, latit_ori, longit_ori, gearClass, mobility) |>

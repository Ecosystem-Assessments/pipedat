...
dp_params <- function() {
  c(
    ...
    "@param grd raster grid used for data integration, either a path to a `.tif` on disk or a `stars` objects. Default is set to `data/grid/grid.tif`, which is generated by `pipegrid()` or `pipeflow()`",
    "@param integrate logical, whether data should be integrated in a study grid, if available for specific pipeline. Default is set to `TRUE`.",

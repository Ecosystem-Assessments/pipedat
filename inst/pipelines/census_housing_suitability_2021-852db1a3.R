...
#' dp_852db1a3()
#' }
dp_852db1a3 <- function(bbox = NULL, bbox_crs = NULL, timespan = NULL, ...) {
  ...
  if (!exist$clean) {
    ...
    # IMPORT DATA
    # =~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~= #
    utils::unzip(here::here(path, "raw", "98100237-eng.zip"), exdir = here::here(path, "raw"))
    dat <- vroom::vroom(here::here(path,"raw","98100237.csv"))
    # _________________________________________________________________________________________ #
    ...
    unlink(here::here(path, "raw", "98100237.csv"), recursive = TRUE)

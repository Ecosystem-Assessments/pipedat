#' @eval get_name("71944efd")
#'
#' @eval get_description("71944efd")
#'
#' @eval dp_params()
#'
...
#'
#' @keywords pipeline_id: 71944efd
#'
#' @examples
#' \dontrun{
#' dp_71944efd()
#' }
dp_71944efd <- function(bbox = NULL, bbox_crs = NULL, timespan = NULL, ...) {
  ...
  # =~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~= #
  if (!exist$raw) {
    govcan <- "5577393c-5eb2-4d07-a64e-d2a1b675a242"
    pipeload(
      govcan = govcan,
  ...
  if (!exist$clean) {
    ...
    files <- dir(here::here(path, "raw", "Tiffall"), pattern = ".tif$")

    # Select only surface and bottom
    # NOTE: remove this if you wish to keep all the depths available
    idd <- stringr::str_detect(files, "_0m") |
      stringr::str_detect(files, "Bottom")
    files <- files[idd]

    # Import
    dat <- lapply(here::here(path, "raw", "Tiffall", files), stars::read_stars, proxy = TRUE)
    # _________________________________________________________________________________________ #
    ...
    # =~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~= #
    # Formatted data
    name <- gsub("NorthwestAtlantic_Temperature_", "", files) |>
